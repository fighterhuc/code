<template>
	<section>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="department" label="科室" width="180" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item>
					<el-input v-model="editForm.id" auto-complete="off" v-if="false"></el-input>
				</el-form-item>
	...
	export default {
		...
		methods: {
			...
			getUsers() {
				...
				//NProgress.start();
                httpPost("listUser",para,this).then((data) => {
			...
			handleDel: function (index, row) {
				this.$confirm('确认删除该记录吗?', '提示', {
					...
					//NProgress.start();
					let para = { id: row.id };
                    httpPost("delUser",para,this).then((res) => {
						this.listLoading = false;
						//NProgress.done();
						this.$message({
							message: '删除成功',
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            httpPost("editUser",para,this).then((res) => {
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            httpPost("addUser",para,this).then((res) => {
